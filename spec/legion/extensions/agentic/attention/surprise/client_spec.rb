...
RSpec.describe Legion::Extensions::Agentic::Attention::Surprise::Client do
  subject(:client) { described_class.new }

  describe '#initialize' do
    it 'creates a default store' do
      expect(client.store).to be_a(Legion::Extensions::Agentic::Attention::Surprise::Helpers::SurpriseStore)
    end

    it 'creates a default habituation_model' do
      expect(client.habituation_model).to be_a(Legion::Extensions::Agentic::Attention::Surprise::Helpers::HabituationModel)
    end

    it 'accepts an injected store' do
      custom = Legion::Extensions::Agentic::Attention::Surprise::Helpers::SurpriseStore.new
      c = described_class.new(store: custom)
      expect(c.store).to equal(custom)
    end

    it 'accepts an injected habituation_model' do
      custom = Legion::Extensions::Agentic::Attention::Surprise::Helpers::HabituationModel.new
